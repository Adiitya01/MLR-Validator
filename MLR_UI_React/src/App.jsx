...
function App() {
  ...
  const handleExtractCitations = async () => {
    ...
    try {
      const results = await apiClient.runPipeline(brochureFile, referenceFiles.map(f => f.file))
  ...
  return (
    <ErrorBoundary>
      ...
      <SpecialUseCasesSidebar
        ...
        onDrugsClick={() => {
          // Show notification that DRUG pipeline is activated
          setDrugPipelineNotification(true)
          // Auto-hide after 5 seconds
          setTimeout(() => setDrugPipelineNotification(false), 5000)
        }}
      />
      ...
      {drugPipelineNotification && (
        <div style={{
          ...
          <span>You are using <strong>DRUG PIPELINE</strong> from BACKEND</span>
          <button
